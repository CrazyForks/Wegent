...
2. **⚙️ 双引擎架构**：底层支持 Agno 和 Claude Code 两个 Agent 执行引擎，上层支持对话和编码两种模式
...
- **🎯 零代码智能体团队**：通过网页 UI 构建和部署智能体团队，无需编码 - 只需配置即可运行
- **🚀 双引擎架构**：利用 Agno 处理对话工作流，Claude Code 处理开发任务
- **🔐 生产级隔离**：独立沙箱确保并发团队操作的稳定性和安全性
- **🤖 标准化与声明式**：Kubernetes 风格的 YAML 配置实现通用 AI 智能体管理
- **🌐 多模式灵活性**：在对话模式（分析、研究）和编码模式（开发、审查）之间无缝切换
- **🔗 无缝 Git 集成**：原生支持 GitHub/GitLab，实现自动化代码工作流
